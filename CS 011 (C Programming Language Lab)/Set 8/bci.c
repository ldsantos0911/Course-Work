...
void do_load(int n)
{
    if(n < 0 || n >= NREGS)
    {
        fprintf(stderr, "Accessing non-existent register!\n");
        exit(1);
    }
    do_push(vm.reg[n]);
}
...
void do_jmp(int n)
{
    if(n < 0 || n >= MAX_INSTS)
    {
        fprintf(stderr, "Accessing out-of-bounds memory!\n");
        exit(1);
    }
    vm.ip = (unsigned short) n;
}
...
void do_add(void)
{
    ...
    {
    int first, second;
    first = vm.stack[vm.sp - 1];
    second = vm.stack[vm.sp - 2];
    do_pop();
    do_pop();
    do_push(first + second);
    }
}
